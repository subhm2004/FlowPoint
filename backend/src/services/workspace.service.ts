...
export const removeWorkspaceMemberService = async (
  ...
  if (targetUserId === String(requesterUserId)) {
  ...
  if (
    removedUser?.currentWorkspace &&
    removedUser.currentWorkspace.equals(wsObjectId)
  ) {
    const nextMembership = await MemberModel.findOne({ userId: targetUserId });
    removedUser.currentWorkspace = nextMembership
      ? (nextMembership.workspaceId as mongoose.Types.ObjectId)
      : null;
